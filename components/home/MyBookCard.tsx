import { StyleSheet, Text, View, Dimensions, Animated, TouchableOpacity, Image, Alert } from "react-native";
...
const MyBookCard = ({item,style}:any) => {
  ...
  useFocusEffect(() => {
    const bookprogress=myBooksProgress.find((b: any) => b.id === item.bookDetails.bookName && b)
  ...
  const handleReturnBook=()=>{
    Alert.alert(
      ...
      [
        ...
        { text: "Return", onPress: () => returnBookService(user?._id,item.bookID),style:"default" },
      ],
    )}
  ...
  return (
    <TouchableOpacity onPress={handlePress} style={[styles.cardContainer,style]}>
      ...
      <CustomText fontFamily="Medium" variant="h5">{item.bookDetails.bookName}</CustomText>
...
const styles = StyleSheet.create({
  ...
  returnBtn:{flexDirection:'row',borderWidth:1,borderRadius:10,width:108,alignItems:'center',padding:3,gap:3},
  sliderBG: {
    width: "100%",
    height: 10,
    borderRadius: 5,
